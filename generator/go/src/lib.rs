...
struct NType<'a>(&'a Type<'a>);
struct NArray<'a>(&'a Array<'a>);
struct NEndian<'a>(&'a Endian);
struct NStruct<'a>(&'a Struct<'a>);

impl fmt::Display for NType<'_> {
            ...
            match &arr {
                Array::Unknown(arr_kind) => {
                    if item.kind == Type::CString {
                        ...
                        "\t" "err = stream.Write" (alt(arr_kind)) "(" (NType(arr_kind)) "(len(" (var_name) "." (item.name) ") + 1)" (type_write_border(arr_kind, item.byte_order)) ")" "\n"
                    } else {
                        "\t" "err = stream.Write" (alt(arr_kind)) "(" (NType(arr_kind)) "(len(" (var_name) "." (item.name) "))" (type_write_border(arr_kind, item.byte_order)) ")" "\n"
                    }
                    "\t" "if err != nil {" "\n"
                    "\t\t" "return err" "\n"
                    "\t" "}" "\n"
    ...
    if let Some(arr) = &item.array {
        ...
        fomat!(
            match &arr {
                Array::Constant(size) => {
                    if emit_for_loop {
                        "\t" (var_name) "." (item.name) " = make([]" (NType(&item.kind)) ", " (size) ")" "\n"
                    }
                }
                Array::Variable(size_name, _) => {
                    if emit_for_loop {
                        "\t" (var_name) "." (item.name) " = make([]" (NType(&item.kind)) ", " (var_name) "." (size_name) ")" "\n"
                    }
                }
                Array::Unknown(arr_kind) => {
                    ...
                    "\t" "}" "\n"
                    if emit_for_loop {
                        "\t" (var_name) "." (item.name) " = make([]" (NType(&item.kind)) ", " (var_name) (some_kind_of_uppercase_first_letter(item.name)) "Size)" "\n"
                    }
                }
            }
            if emit_for_loop {
        ...
        wite!(
            ...
            for item in &self.0.items {
                match &item.array {
                    Some(arr) => {
                        if item.kind == Type::CString {
                            "\t" (item.name) "\t" (NType(&item.kind)) "\n"
                        } else {
                            "\t" (item.name) "\t" (NArray(arr)) (NType(&item.kind)) "\n"
                        }
                    }
                    None => { "\t" (item.name) "\t" (NType(&item.kind)) "\n" }
                ...
                if item.kind != Type::CString {
                    match &item.array {
                        Some(Array::Constant(size)) => {
                            "\t" "res." (item.name) " = make([]" (NType(&item.kind)) ", " (size) ")" "\n"
                        }
                        _ => {}
                    }
                }
